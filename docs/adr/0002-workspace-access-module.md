...
**Consequences:** `WorkspaceGuard` only enforces `@WorkspaceRoles()` after resolve. `getWorkspaceForUser` reuses the same resolve path when context is not already on the request.
