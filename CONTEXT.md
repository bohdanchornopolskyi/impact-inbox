...
**Template settings**:
Subject, preheader, width, colors, and fonts on a template — part of the **Working copy** content bundle, snapshotted into each **Template revision** with the block tree. Template width is 480–700px at launch (default 600px). New templates are seeded from the workspace **Brand kit**; per-template edits afterward do not change the kit.
_Avoid_: Settings stored only on revision rows, settings split across separate template columns, width outside 480–700px at launch, live-linking template colors back to the brand kit
...
**Template builder**:
The visual editor for template **Working copy**. v1: all **Block type** entries from `TEMPLATE_BLOCK_DEFINITIONS` are addable from the palette; property panels may be minimal or schema-driven — polish per block type later. Layout is section → row → column → content blocks. Canvas DnD (ADR 0013) is the target manipulation model: sections, rows, columns, and content blocks can be reordered on the canvas, and new blocks can be dragged from the palette to a specific valid drop target. Palette click-to-add remains as a fallback using the current layout context. The Structure panel remains a secondary advanced tool, not the only way to place blocks. Builder-only chrome stays minimal: hover/selection outlines, empty-container hints, and drop indicators while dragging; no always-on wireframe scaffolding. Preview: desktop canvas at template width (default 600px) and mobile viewport toggle (~375px) — same rendered HTML, no merge-tag sample contact in v1. Template width is configurable in **Template settings** (480–700px). **Working copy** autosaves (debounced); explicit Save creates a **Template revision** for version history. **Template list preview** is a cached visual snapshot of the template, updated on explicit Save — not on autosave; list shows placeholder until first Save. Canvas polish (ADR 0008): iframe bridge for click-to-select, selection chrome, and inline edit. `heading`/`text` edit as plain text in-place; `button` is canvas-selectable but edited in the inspector only; `richtext` edits in-place inside the iframe (the rendered element becomes `contenteditable`, so it inherits the real email styles — no parent overlay, no coordinate sync), while the **formatting toolbar** (bold, italic, underline, links, lists, paragraph/heading h1–h6) and **settings** live in the sidebar and drive the in-iframe selection via `document.execCommand`; HTML sanitized on commit. Preview refetch pauses during inline edit and commits on blur/click-away (heading/text) or when leaving a richtext block; richtext HTML autosaves on each canvas change via `block-edit-sync`; Escape cancels richtext edit. Full-screen Preview overlay stays read-only and has no builder bridge. Block actions (ADR 0019): the canvas selection toolbar carries **Move**, **Duplicate**, and **Delete**; the inspector shows Duplicate/Remove for content *and* layout blocks. **Duplicate** inserts a copy directly after the original with fresh ids on the whole subtree (`Ctrl/Cmd+D`, one undo step, new copy selected); duplicating a column redistributes row widths. The merge-tag picker in **Template settings** inserts at the caret in subject/preheader (last-focused wins) and copies to the clipboard everywhere else. Templates duplicate from the list menu — the copy takes the working copy, name ` (copy)`, and list preview, and starts with **no revision history**.
_Avoid_: Curated palette that omits registered blocks, bespoke-only editor with no shared block registry, WYSIWYG HTML-as-primary mode, arbitrary free-form positioning outside section → row → column → content, always-on layout wireframes, desktop-only preview, separate mobile render pipeline, manual save required for working copy, autosave creating revisions, list previews rendered on every list load, hard-coded 600px with no user override, duplicating block render logic in the builder instead of iframe preview, a parent-document richtext editor overlaid on the iframe (coordinate/style sync hell), bridge script on the read-only Preview overlay, block copies that share ids with the original, template copies that inherit revision history, per-mutation bridge messages where `builder-shortcut` already carries the intent

**Brand kit**:
Workspace-owned design tokens for email creation: colors (primary, on-primary, text, heading, page/content backgrounds, link), logo URL, font family (optional type scale), and spacing scale (section padding, content gap, button radius/padding). Edited under workspace settings by workspace admin/owner and org owner. When creating a template or inserting a block, resolved values are **baked** into **Template settings** / block props — later kit changes do not rewrite existing working copies. Platform defaults apply when kit fields are unset. Logo may be a URL until **Object storage** upload. Saving the kit shows a short notice that new blocks/templates will use it. See [ADR 0018](./docs/adr/0018-workspace-brand-kit-and-saved-modules.md).
_Avoid_: Org-wide brand in v1, live token restyle of existing blocks, brand edited only inside one template, requiring upload before URL logo works

**Saved module**:
A named, workspace-scoped reusable block subtree (usually a section) stored in the workspace module library. Inserting into a template **clones** the subtree — edits are local to that working copy. Users can save a selection into the library. New workspaces get platform starters (header, footer, CTA band at minimum); insert prefills workspace name, physical address, and brand logo when available. Same edit permissions as **Brand kit**. See [ADR 0018](./docs/adr/0018-workspace-brand-kit-and-saved-modules.md).
_Avoid_: Linked modules that update every template on library edit, org-global module library in v1, starters that mutate a shared global row when customized
