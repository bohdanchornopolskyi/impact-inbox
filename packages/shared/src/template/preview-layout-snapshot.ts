...
import { buildColumnBlockIdMap } from "./walk-content-blocks";
  ...
  return body.map((section) => ({
    ...
    children: section.children.map((row) => ({
      ...
      children: row.children.map((column) => ({
        ...
        children: blockIdMap.get(column.id) ?? [],
      })),
    })),
  }));
}
...
export function getPreviewLayoutKey(content: TemplateContentData): string {
  const blockIdMap = buildColumnBlockIdMap(content);

  return JSON.stringify({
    settings: content.settings,
    layout: snapshotLayout(content.body, blockIdMap),
  });
}
